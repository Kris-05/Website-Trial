import React, { useEffect, useRef, useState } from 'react'
import Button from './Button';
import { TiLocationArrow } from 'react-icons/ti';
import { useWindowScroll } from 'react-use';
import gsap from 'gsap';

const navItems = ['Nexus', 'Vault', 'Prolouge', 'About', 'Contact'];

const Navbar = () => {

  const [isAudio, setIsAudio] = useState(false); // for playing audio
  const [isIndicator, setIsIndicator] = useState(false); // for active indicator
  const [lastScrollY, setLastScrollY] = useState(0);
  const [isVisible, setIsVisible] = useState(true); // for navbar visibility

  const { y: currentScrollY } = useWindowScroll();

  const navRef = useRef(null);
  const audioRef = useRef(null);
  const toggleAudioIndicator = () => {
    setIsAudio((prev) => !prev)
    setIsIndicator((prev) => !prev)
  }

  useEffect(() => {
    if(isAudio){
      audioRef.current.play();
    } else {
      audioRef.current.pause();
    }
  }, [isAudio])
 
  useEffect(() => {
    if(currentScrollY == 0){
      setIsVisible(true);
      navRef.current.classList.remove('floating-nav');
    } else if (currentScrollY > lastScrollY) {
      setIsVisible(false);
      navRef.current.classList.add('floating-nav');
    } else if (currentScrollY < lastScrollY) {
      setIsVisible(true);
      navRef.current.classList.add('floating-nav');
    }

    setLastScrollY(currentScrollY);
  }, [currentScrollY, lastScrollY])

  useEffect(() => {
    gsap.to(navRef.current, {
      y: isVisible ? 0 : -100,
      opacity: isVisible ? 1 : 0,
      duration: 0.2
    })
  })

  return (
    <div ref={navRef} className='fixed inset-x-0 top-4 z-50 h-16 border-none transition-all duration-700 sm:inset-x-6'>
      <header className='absolute top-1/2 w-full -translate-y-1/2'>
        <nav className='flex size-full items-center justify-between p-4'>
          <div className='flex items-center gap-7'>
            <img src="img/logo.png" alt="logo" className='w-10'/>
            <Button 
              id="product-button" title="Products"
              rightIcon={<TiLocationArrow/>}
              containerClass="bg-blue-50 md:flex hidden items-center justify-center gap-1"
            />
          </div>
          <div className='flex h-full items-center'>
            <div className='hidden md:block'>
              {navItems.map((items) => (
                <a key={items} href={`#${items.toLowerCase()}`} className='nav-hover-btn'>
                  {items}
                </a>
              ))}
            </div>

            <button className='ml-10 flex items-center space-x-1' onClick={toggleAudioIndicator}>
              <audio ref={audioRef} 
                className='hidden' src="audio/loop.mp3" loop
              />
              {[1, 2, 3, 4].map((bar) => (
                <div key={bar} 
                  className={`indicator-line !w-0.5 ${isIndicator ? 'active' : ''}`} 
                  style={{ animationDelay: `${bar * 0.1}s`}}
                />
              ))}
            </button>
          </div>
        </nav>
      </header>
    </div>
  )
}

export default Navbar
